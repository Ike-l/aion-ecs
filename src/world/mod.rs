...
impl World {
    pub fn check_query<Q: Query>(&self) -> Option<HashMap<Vec<TypeId>, Access>> {
    ...
    pub fn prepare_query<Q: Query>(&self) -> Option<(PreparedQuery<Q>, HashMap<Vec<TypeId>, Access>)> {
    ...
    pub fn finalise_query(&self, access_ids: &HashMap<Vec<TypeId>, Access>) {
        let mut tracking = self.tracking.write();
